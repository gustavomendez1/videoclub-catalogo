    ...
    <div class="max-w-2xl mx-auto my-10 p-4">
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden" style="background-color: #fff; border-radius: 0.75rem; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); border: 1px solid #f3f4f6; overflow: hidden;">
            ...
            <div class="p-6 bg-gray-50/50" style="padding: 1.5rem;">
                <form action="{{ url('/catalog/edit/' . $id) }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 1.25rem;">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" class="form-control" value="{{ $pelicula->title }}" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;" required>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 1rem;">
                        <div>
                            ...
                            <input type="text" name="year" id="year" class="form-control" value="{{ $pelicula->year }}" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; text-align: center;" required>
                        </div>
                        <div>
                            ...
                            <input type="text" name="director" id="director" class="form-control" value="{{ $pelicula->director }}" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem;" required>
                        </div>
                    </div>

                    <div>
                        <label for="poster" style="display: block; font-size: 0.875rem; font-weight: 600; color: #374151; margin-bottom: 0.25rem;">Póster (Subir nuevo archivo)</label>
                        <input type="file" name="poster" id="poster" class="form-control" accept="image/*" style="width: 100%; padding: 0.375rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="synopsis" id="synopsis" rows="4" class="form-control" style="width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; resize: none;" required>{{ $pelicula->synopsis }}</textarea>
                    </div>

                    <div style="border-top: 1px solid #e5e7eb; padding-top: 1rem; display: flex; justify-content: space-between; align-items: center;">
                        <a href="{{ url('/catalog/show/' . $id) }}" style="text-decoration: none; color: #4b5563; font-weight: 500; font-size: 0.875rem;">❌ Cancelar</a>
